...
class CameraStream:
    ...
    def face_detection_process(frame_queue, result_queue, face_process_interval, face_data, camera_stream):
        ...
        while True:
            ...
            if frame_count % face_process_interval == 0:
                ...
                if len(face_locations) != len(camera_stream.last_face_locations):
                    ...
                    for i, (top, right, bottom, left) in enumerate(face_locations):
                        ...
                        if abs(top - last_top) > 5 or abs(right - last_right) > 5 or abs(bottom - last_bottom) > 5 or abs(left - last_left) > 5:
                            new_faces_detected = True
                            break
            ...
            for (left, top, right, bottom), name in last_face_data.items():
